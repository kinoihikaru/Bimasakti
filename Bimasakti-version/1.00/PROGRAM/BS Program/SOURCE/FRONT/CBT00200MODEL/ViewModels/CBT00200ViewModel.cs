    ...
    public class CBT00200ViewModel : R_ViewModel<CBT00200DTO>
    {
        ...
        public async Task GetAllUniversalData()
        {
            ...
            try
            {
                ...
                //Set Dept Code
                if (loDeptResult.Any(x => x.CDEPT_CODE == loResult.VAR_GL_SYSTEM_PARAM.CCLOSE_DEPT_CODE))
                {
                    JornalParam.CDEPT_CODE = loResult.VAR_GL_SYSTEM_PARAM.CCLOSE_DEPT_CODE;
                    JornalParam.CDEPT_NAME = loResult.VAR_GL_SYSTEM_PARAM.CCLOSE_DEPT_NAME;
                }

                //Set Journal Period
                if (!string.IsNullOrWhiteSpace(loResult.VAR_CB_SYSTEM_PARAM.CSOFT_PERIOD_YY))
                    JournalPeriodYear =  int.Parse(loResult.VAR_CB_SYSTEM_PARAM.CSOFT_PERIOD_YY);

                JournalPeriodMonth = loResult.VAR_CB_SYSTEM_PARAM.CSOFT_PERIOD_MM;
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
