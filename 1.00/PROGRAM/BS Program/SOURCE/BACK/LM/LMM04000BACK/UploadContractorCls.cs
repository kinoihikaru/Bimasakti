...
{
    public class UploadContractorCls : R_IBatchProcess
    {
        public List<UploadContractorDTO> GetUploadContractorList(List<UploadContractorDTO> poEntity)
        {
            ...
            try
            {
                ...
                loDb.R_BulkInsert<UploadContractorDTO>((SqlConnection)loConn, "#TENANT", poEntity);

                lcQuery = "UPDATE A SET A.Var_Exists = 1 " +
                    "FROM #TENANT A WHERE EXISTS " +
                    "(SELECT TOP 1 1 FROM LMM_TENANT B " +
                    "WHERE B.CCOMPANY_ID = A.CompanyId " +
                    "AND B.CPROPERTY_ID = A.PropertyId " +
                    "AND B.CCUSTOMER_TYPE  = '02' " +
                    "AND B.CTENANT_ID = A.TenantId)";


                loDb.SqlExecNonQuery(lcQuery, loConn, false);

                lcQuery = "SELECT * FROM #TENANT";
                ...
                loResult = R_Utility.R_ConvertTo<UploadContractorDTO>(loDataTable).ToList();
            }
            catch (Exception ex)
            {
                loException.Add(ex);
        ...
        public void _BatchProcess(R_BatchProcessPar poBatchProcessPar)
        {
            R_Db loDb = new R_Db();
            DbConnection loConn = null;
            DbCommand loCmd = null;
            ...
            try
            {
                ...
                var loObject = R_NetCoreUtility.R_DeserializeObjectFromByte<List<UploadContractorDTO>>(poBatchProcessPar.BigObject);
                ...
                List<UploadContractorSaveDTO> loParam = new List<UploadContractorSaveDTO>();
