    ...
    public class LMM01000Cls : R_BusinessObject<LMM01000DTO>
    {
        ...
        public List<LMM01000DTOPropety> GetProperty(LMM01000PropertyParameterDTO poEntity)
        {
            ...
            try
            {
             ...
             .Where(x => x != null && x.ParameterName.StartsWith("@")).Select(x => x.Value);
                _LMM01000logger.LogDebug("EXEC RSP_GS_GET_PROPERTY_LIST {@poParameter}", loDbParam);
        ...
        public LMM01000PrintDTO GetBaseHeaderLogoCompany(LMM01000PrintParamDTO poEntity)
        {
            ...
            try
            {
                ...
                var loDbParam = loCmd.Parameters.Cast<DbParameter>()
                .Where(x => x != null && x.ParameterName.StartsWith("@")).Select(x => x.Value);
                _LMM01000logger.LogDebug("SELECT dbo.RFN_GET_COMPANY_LOGO({@CCOMPANY_ID}) as CLOGO", loDbParam);

                var loDataTable = loDb.SqlExecQuery(loConn, loCmd, true);
                loResult = R_Utility.R_ConvertTo<LMM01000PrintDTO>(loDataTable).FirstOrDefault();
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
