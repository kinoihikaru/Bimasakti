...
namespace LMM06500FRONT
{
    public partial class LMM06500 : R_Page
    {
            ...
            try
            {
                ...
                if (eventArgs.ConductorMode == R_eConductorMode.Edit)
                {
                    if (loOldData == "02" && loData.CPOSITION != loOldData)
                    {
                        ...
                        {
                            await _Staff_viewModel.ActiveInactiveProcessAsync(loData);
                            await _Staff_conductorRef.R_SetCurrentData(loData);
                        }
        ...
        private async Task _Staff_InActive_Onclick()
        {
            ...
            try
            {
                var loValidate = await R_MessageBox.Show("", string.Format(R_FrontUtility.R_GetMessage(typeof(Resources_Dummy_Class), "_NotifInactive"), _Staff_lcLabel, loGetData.CSTAFF_ID), R_eMessageBoxButtonType.YesNo);
        ...
        private async Task _Staff_City_After_Open_Popup(R_AfterOpenLookupEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                var loData = (LMM06500DTO)_Staff_conductorRef.R_GetCurrentData();
                loData.CCITY = loTempResult.CCODE;
                loData.CPROVINCE = loTempResult.CCODE_PROVINCE;
                loData.CCOUNTRY = loTempResult.CCODE_COUNTRY;
            }
            catch (Exception ex)
            {
                loEx.Add(ex);
