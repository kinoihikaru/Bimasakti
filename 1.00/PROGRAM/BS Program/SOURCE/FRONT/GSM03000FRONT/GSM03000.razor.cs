    ...
    public partial class GSM03000 : R_Page
    {
        ...
        private async Task Additional_Validation(R_ValidationEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                if (eventArgs.ConductorMode == R_eConductorMode.Add && loGetDataParam.LACTIVE)
                {
                    ...
                    {
                        await Additional_viewModel.ActiveInactiveProcessAsync(loParam); //Ganti jadi method ActiveInactive masing masing
                        await Additional_conductorRef.R_SetCurrentData(loGetDataParam);
                        return;
                    }
        ...
        private async Task Additional_R_After_Open_Popup_ActivateInactive(R_AfterOpenPopupEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                if (result == true)
                {
                    ...
                    await Additional_conductorRef.R_SetCurrentData(loGetData);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task GridDeducation_Validation(R_ValidationEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                if (eventArgs.ConductorMode == R_eConductorMode.Add && loGetDataParam.LACTIVE)
                {
                    ...
                    if (loValidateViewModel.loRspActivityValidityList.FirstOrDefault().CAPPROVAL_USER == "ALL" && loValidateViewModel.loRspActivityValidityResult.Data.FirstOrDefault().IAPPROVAL_MODE == 1)
                    {
                        await Deducation_viewModel.ActiveInactiveProcessAsync(loParam); //Ganti jadi method ActiveInactive masing masing
                        await Deducation_conductorRef.R_SetCurrentData(loGetDataParam);
                        return;
                    }
                    else //Disini Approval Code yang didapat adalah 2, yang berarti Active Inactive akan dijalankan jika User yang diinput ada di RSP_ACTIVITY_VALIDITY
                    {
        ...
        private async Task GridDeducation_R_After_Open_Popup_ActivateInactive(R_AfterOpenPopupEventArgs eventArgs)
        {
            ...
            try
            {
                ...
                if (result == true)
                {
                    var loData = await Deducation_viewModel.ActiveInactiveProcessAsync(loParam);
                    await Deducation_conductorRef.R_SetCurrentData(loGetData);
                }
            }
            catch (Exception ex)
            {
